...
| INFRA-FORSOK1 Serverägd försökslivscykel | order `5847195010` | ritten (`HorseService`, `rittId`), `VoltObservation` | försök som serverägd post före instruktörens konsumenter | INFRA-VOLT1 | `RidForsok` + server-API i HorseService (`startaForsok`, `avbrytForsok`, `forsok`, `forsokVolt`; ingen RemoteEvent): servergenererat `forsokId`, ritt/häst/ryttare ur sessionen, övning `storvolt` med bara serverdefinitionen `server-referens-1` (klientens v1 nekas); ägare och AKTUELL accepterad ritt härleds ur `byRider`/sessionen (saknad, stängd, gammal, annan spelares ritt och inaktuell bokföring nekas utan mutation); ett aktivt per ritt; sekvensbaserad dublett/retry (samma sekvens → samma utfall, äldre okänd → `inaktuell_begaran`), 16 utfall per ritt; stängs exakt en gång med ritten (avsittning, död, lämnande, avregistrering), nekad avsittning bevarar, uppsittning startar inget; start-/slutmarkörer (ritt-tid, voltens högsta avsnitts-nr) så att voltavsnitt som pågick vid starten aldrig tillskrivs, `forsok.historik_gallrad` när underlaget gallrats; frysta bilder utan resultatfält (`bedomning = "ingen"`), 8 försök per ritt, ring om 32 ritter; ingen sparning (mätt), ingen poäng, ingen klient; 25 mutationer röda + 1 ekvivalent (`ridforsok.spec`, 73 OK, KOHERENS-bänken, BANK_ONLY) | — | `BYGGT` + `INTEGRERAT` i HorseService (servergrund, lokalt; ej granskat). PENDING: instruktörskonsument, klient/UI, uppgiftsbaserat lektionsavslut, bedömning och progression |
