...
| Uteridbanan | 176 / 119 | långsidan nord–syd | 20 × 40 `[ASSUMPTION]` | `[REFERENCE GAP]` **Måtten och läget är olösta.** 33,57 m är ett `MEASURED SIDE` på "den stora nordöstra träningsytan", men det är INTE dokumenterat att den ytan är spelets uteridbana. Måttet hann sättas som kortsida och är ÅTERTAGET: identiteten måste dokumenteras först. 20 × 40 stödjs inte av någon källa och ska inte läsas som dressyrmått. Staket, master, domarkur och torvbalar läser rektangeln, så måtten behöver bara skrivas på ett ställe när de kommer. |
