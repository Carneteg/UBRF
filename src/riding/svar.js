...
function svarSvarstid(h, fokus, energi, klarhet) {
  ...
  const t = S.SVAR_BAS
    ...
    + S.SVAR_ENERGI * (1 - kl(energi, 0, 1))
    - S.SVAR_KLAR * kl(klarhet, 0, 1);
...
function svarEnergiTakt(gangart, spanning) {
  const S = SVAR_KANON;
  const tapp = (S.ENERGI_TAPP[gangart] || 0) * (1 + S.ENERGI_SPANN * (spanning || 0));
  const ater = S.ENERGI_ATER[gangart] || 0;
  return ater - tapp;
}
