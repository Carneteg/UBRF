...
function lararSteg(dt){
  ...
  if(LARARE.brasedan>20&&LARARE.beromt<3){
    ...
    /* Ledarens egna berömrepliker blandas in — Sofie jublar, Bengt
       säger "Godkänt". Motorn bestämmer att beröm sägs, inte orden. */
    return lararRad(F.beroem.concat(
      typeof ledarTemaBerom==="function"?ledarTemaBerom():[]));
  }

  if(LARARE.cd>0)return "";
  /* Pausen färgas av vem som håller lektionen: pratiga ledare ropar
     tätare, fåordiga väntar längre. Pedagogiken är densamma. */
  LARARE.cd=(12+Math.random()*5)*(typeof ledarTakt==="function"?ledarTakt():1);
  ...
  /* Rättelsen hålls till dagens tema, men ledarens egna repliker för
     just det temat läggs till som varianter (ledare.js). */
  return lararRad(F.ratta.concat(
    typeof ledarTemaRattor==="function"?ledarTemaRattor(F.id):[]));
}
